        ...
        if ('lat' or 'long') in post:
            ...
            try:
                ...
                if ('emp' and 'attend') in post:
                    ...
                    emp_id = request.env['hr.employee'].sudo().search(
                        [('id', '=', int(post['emp']))], limit=1)
                    if emp_id.attendance_state == 'checked_in':
                        attendance_id.write({
                        ...
                        attendance_id.write({
                            'logout_address': str(reverse[0]),
                            'logout_browser': str(post['browser']),
                            'logout_os': str(post['os'])
                        })
            except IndexError:
                return json.dumps([])
...
class HrAttendanceInherit(models.Model):
    ...
    logout_address = fields.Text('Logout Location', readonly=True)
    logout_browser = fields.Char('Logout Browser', readonly=True)
    logout_os = fields.Char('Logout OS Details', readonly=True)
